...
class Utils:
    ...
    def count_digits(self, data: list[int]) -> dict:
        """Count occurrences of all digits (0–9) in the data"""
    ...
    def get_expected_percentages(self) -> dict:
        ...
        expected = [0.301, 0.176, 0.125, 0.097,
    ...
    def get_p_value(self, data: list[int]) -> tuple:
        ...
        expected_proportions = [0.301, 0.176, 0.125,
                                0.097, 0.079, 0.067, 0.058, 0.051, 0.046]
        expected_counts = [total_observed *
                           prop for prop in expected_proportions]
        chi2_stat, p_value = chisquare(
            list(observed_counts.values()), expected_counts)
    ...
    def get_ks_test(self, data: list[int]) -> tuple:
        ...
        observed_counts = self.count_digits(data)
        observed_proportions = [observed_counts[digit] /
                                total_observed for digit in range(1, 10)]
        cumulative_observed = np.cumsum(observed_proportions)
        cumulative_expected = np.cumsum(expected_proportions)
        ks_statistic = np.max(
            np.abs(cumulative_observed - cumulative_expected))
    ...
    def get_mad(self, data: list[int]) -> float:
        ...
        observed_proportions = [observed_counts[digit] /
                                total_observed for digit in range(1, 10)]
        absolute_deviations = [
            abs(o - e) for o, e in zip(observed_proportions, expected_proportions)]
